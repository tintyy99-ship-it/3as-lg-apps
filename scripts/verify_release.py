    ...
    for apk, tag in [(eleve_apk, "eleve"), (admin_apk, "admin")]:
        ...
        if tag == "eleve":
            ...
            for feat in ["viewList", "playDoors", "setLang", "toggleDrawer", "MAT_COEF",
                         "slideBlur", "slideBlur".lower(), "__booted", "lg-out", "1.10"]:
                check(f"eleve: feature {feat}", feat.lower() in html.lower(), "")
        else:
            for feat in ["suspendre", "supprimerEleve", "deleteCourse", "deleteSectionUI", "deleteMatiereUI", "deleteChat", "askDeleteAll", "__booted",
                         "toast", "1.10", "5db4f9e3587a9752"]:
